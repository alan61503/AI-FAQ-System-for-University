...
export const INTERNAL_KB_FAQS: InternalKbFaq[] = [
  {
  ...
  },
  {
    ...
    answer:
      "Eligibility varies by program. Typical requirements include completion of the prior qualifying degree, minimum percentage, and any program-specific subject prerequisites.",
  ...
  },
  {
    ...
    answer:
      "Common documents include transcripts/marks cards, ID proof, passport-size photos, migration/transfer certificate (if applicable), and category certificates.",
  ...
  },
  {
    ...
    answer:
      "Apply online, submit documents, attend any required entrance test/interview, receive an offer, and complete fee payment to confirm the seat.",
  ...
  },
  {
    ...
    answer:
      "Some programs require an entrance test and/or interview. Check your program page for the exact selection components.",
  ...
  },
  {
    ...
    answer:
      "The application fee is program-specific and is paid during online submission. The fee is generally non-refundable.",
  ...
  },
  {
    ...
    answer:
      "Typical fee pattern:\n\n• 1st Year: ₹2,00,000\n• 2nd Year: ₹2,00,000\n• 3rd Year: ₹2,30,000\n• 4th Year: ₹2,30,000\n\nFor the latest official fee sheet, contact the Admissions or Accounts office.",
  ...
  },
  {
    ...
    answer:
      "The fee structure is published on the admissions portal and in program-specific notifications. For a detailed breakup, contact the Finance/Accounts office.",
...
export function searchInternalKb(query: string) {
  const q = normalize(query);
  if (!q) return null;

  let best: { item: InternalKbFaq; score: number } | null = null;

  for (const item of INTERNAL_KB_FAQS) {
...
export function rankInternalKb(query: string, limit = 5) {
  const q = normalize(query);
  if (!q) return [];

  const scored = INTERNAL_KB_FAQS.map((item) => {
    const hay = normalize(`${item.question} ${item.tags.join(" ")}`);
    let score = 0;
    for (const token of q.split(" ")) {
      if (token.length < 3) continue;
