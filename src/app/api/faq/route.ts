...
import { searchInternalKb, rankInternalKb } from "@/lib/internal-kb";
...
export async function POST(request: Request) {
  ...
  const directInternalKbHit = searchInternalKb(question);
  ...
  try {
    ...
    const internalKbContextItems = rankInternalKb(question, 5)
      .map(
        (item, idx) =>
    ...
    if (fallbackCitations.length === 0 && internalKbContextItems) {
    ...
    const finalAnswer =
      sanitizeFallbackAnswer(
        parsed?.answer ||
          text ||
            (directInternalKbHit?.answer ?? "I could not generate an answer right now.")
      );
    ...
    const payload: FaqResponse = {
      ...
      confidence:
        ...
        (rankedOfficialCitations.length > 0
          ...
          : userDataContext
            ? 0.68
            : directInternalKbHit || internalKbContextItems
              ? 0.52
              : 0.4),
